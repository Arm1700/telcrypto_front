...
export const CryptoTable: React.FC<CryptoTableProps> = ({ prices, isConnected = false }) => {
  ...
  const orderedRows = targetSymbols.map((symbol, idx) => {
    ...
    return item
      ? { ...item, rank: idx + 1 }
      : { symbol, price: null, change24h: null, volume24h: null, marketCap: null, rank: idx + 1 };
  });
            ...
            <tbody>
              {orderedRows.length === 0 ? (
                ...
                orderedRows.map((price, idx) => (
                  <tr key={price.symbol ?? idx} className={styles.cryptoRow}>
                    <td className={styles.rankCell}>{price.rank}</td>
                    <td className={styles.nameCell}>{price.symbol.replace('USDT', '')}</td>
                    <td className={styles.priceCell}>{formatPrice(price.price)}</td>
                    <td className={`${styles.changeCell} ${getChangeClass(price.change24h)}`}>{formatPercent(price.change24h)}</td>
                    <td className={styles.marketCapCell}>{formatPrice(price.marketCap)}</td>
                    <td className={styles.volumeCell}>{formatVolume(price.volume24h)}</td>
                  </tr>
                ))
              )}
            </tbody>
